...
export default {
  ...
  methods: {
    ...
    fileReader: function(file, fileList) {
      ...
      let fileWect =
        fileWectName == "聊天记录"
          ? file.name.split("聊天记录-")[1].split(".")[0]
          : file.name.split("通讯录-")[1].split(".")[0];
    ...
    parserData: function(data, name, filterName) {
      ...
      data.forEach((element, xindex) => {
          ...
          switch (key) {
            ...
            case "微信帐号":
              ...
              if (value && value != "未知好友") {
                ...
                child.senderId = senderArray[senderArray.length - 1];
                var index = value.lastIndexOf("/");
                var str = value.substring(0, index);
                child.senderComments = str;
        ...
        if (this.patrn.exec(child.msgContent)) {
          child.isResource = 1;
          if (fs.existsSync(child.msgContent)) {
            this.filesnumber.push(child.msgContent);
          }
        } else {
          child.isResource = 0;
        }
        if (child.sender !== "未知好友") {
          tableData.push(child);
          // if(groupBy[child.groupName]){
          //   groupBy[child.groupName].push(child)
          // }else{
          //   groupBy[child.groupName]=[child]
          // }
        }
      });
      let allObj = []; //处理之后的数据
      ...
      tableData.map((item, index) => {
        ...
        if (item.msgType == "已接收") {
          count.push(wectname);
          count.push(item.senderId);
        } else {
          for (let i = index; i >= 0; i--) {
            if (
              item.groupName == tableData[i].groupName &&
              tableData[i].msgType == "已接收"
            ) {
              count.push(wectname);
              count.push(tableData[i].senderId);
              break;
            }
          }
          if (count.length == 0) {
            for (let j = index; j < tableData.length; j++) {
              if (
                ...
                count.push(wectname);
                count.push(tableData[j].senderId);
                break;
              }
            }
          }
        ...
        obj =
          count.length == 0
            ? Object.assign({}, item, { consultant: name })
            : Object.assign(
                {},
                item,
                { consultant: name },
      ...
      console.log("导入数据总数", allObj.length);
      this.allObj = allObj;
